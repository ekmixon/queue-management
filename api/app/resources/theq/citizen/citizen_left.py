...
class CitizenLeft(Resource):
    ...
    def post(self, id):
        ...
        my_print(
            f"++> POST API call time before csr = statement: {str(datetime.now())}"
        )

        csr = CSR.find_by_username(g.jwt_oidc_token_info['username'])
        my_print(f"    ++> Time before citizen = statement: {str(datetime.now())}")
        citizen = Citizen.query.filter_by(citizen_id=id).first()

        my_print(f"    ++> Time before citizen ID statement: {str(datetime.now())}")
        citizen_id_string = self.get_citizen_string(citizen)
        citizen_ticket = self.get_ticket_string(citizen)

        my_print(
            f"    ++> Time before citizen ticket statement: {str(datetime.now())}"
        )

        my_print(
            f"    ++> POST /citizens/{citizen_id_string}/citizen_left/, Ticket: {citizen_ticket}"
        )

        my_print(f"    ++> Time before sr_state statement: {str(datetime.now())}")
        ...
        if len(citizen.service_reqs) != 0:
            ...
            status = "at-prep" if active_period.ps.ps_name == "Invited" else "being-served"
        my_print(f"    ++> Time before Snowplow call: {str(datetime.now())}")
        SnowPlow.snowplow_event(
            citizen.citizen_id,
            csr,
            f"left/{status}",
            quantity=quantity,
            current_sr_number=sr_number,
        )


        my_print(
            f"    ++> Time before closing non-active service requests: {str(datetime.now())}"
        )
        ...
        my_print(f"    ++> Time before updating citizen state: {str(datetime.now())}")
        ...
        my_print(
            f"    ++> Time before updating citizen database: {str(datetime.now())}"
        )

        db.session.add(citizen)
        my_print(f"    ++> Time before database commit: {str(datetime.now())}")
        db.session.commit()

        my_print(f"    ++> Time before socket io invited call: {str(datetime.now())}")
        socketio.emit('citizen_invited', {}, room=f'sb-{csr.office.office_number}')
        my_print(f"    ++> Time before creating the result: {str(datetime.now())}")
        result = self.citizen_schema.dump(citizen)
        my_print(f"    ++> Time before socket io update call: {str(datetime.now())}")
        socketio.emit('update_active_citizen', result, room=csr.office_id)

        my_print(f"    ++> Time before return result call: {str(datetime.now())}")
        return {'citizen': result,
                'errors': self.citizen_schema.validate(citizen)}, 200

    def get_citizen_string(self, citizen):
        if citizen is not None:
            return str(citizen.citizen_id) if citizen.citizen_id is not None else "No ID"
        else:
            return "No citizen"

    def get_ticket_string(self, citizen):
        if citizen is not None:
            return citizen.ticket_number if citizen.ticket_number is not None else "None"
        else:
            return "No citizen"
